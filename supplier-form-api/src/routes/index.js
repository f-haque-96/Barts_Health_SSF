 ...
 * Update submission
 */
router.put('/submissions/:id', requireAuth, canAccessSubmission, async (req, res, next) => {
  try {
 ...
 */
router.get('/companies-house/:crn', requireAuth, async (req, res, next) => {
  try {
 ...
 */
router.get('/vendors/check', requireAuth, async (req, res, next) => {
  try {
    const { companyName, vatNumber } = req.query;
 ...
 */
router.post('/documents/:submissionId', requireAuth, canAccessSubmission, upload.single('file'), async (req, res, next) => {
  try {
